...
approximated N-body simulations recover the halos well enough (compared
to a full N-body simulation), we find the same (\textcolor{red}{similar?})
...
the distance conditions does. This implies that deviation of halo
...
halos as a function of halo mass at redshift $z=0.15$. To match a
halo by halo, an algorithm takes a finer resolution sample as a reference
and finds a matched halo from the other sample. Surprisingly, the
number densities for bothe samples are almost the same.
...
simulations is that we can avoid tuning the linking length in order
to get halos correctly.
...
is that smaller time step halos are less dense, and the potential
