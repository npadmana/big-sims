...
\begin{figure}[H]
\includegraphics[width=0.5\columnwidth]{\lyxdot \lyxdot /Plots/unmatchHalo2_content_300_2_z0\lyxdot 15}\includegraphics[width=0.5\columnwidth]{\lyxdot \lyxdot /Plots/unmatchHalo2_content2_300_2_z0\lyxdot 15}
...
\textcolor{blue}{NOTE: We should show an example halo from the simulations
...
and the scatter is sigificantly reduced. The standard deviation in
the differences in halo distances is matched in these cases to better
than $200h^{-1}{\rm kpc}$. \textcolor{red}{The dashed lines represent
...
are shown in Table X. Figure \ref{fig:HaloProperty_step} shows that
the distribution of the differences in positions and velocities are
almost Gaussian (if I find that the distribution fits to Gaussian
perfectly by separating into different masses I will write about it).}
As is clear from Figure \ref{fig:HaloProperty_step}, the difference
between 3 and 2 sub-cycles is negligible on halo properties. Note
that we observe the same trend in halo properties discussed here at
different redshifts and along different axes. 

\textcolor{blue}{NOTE: These distribution should be done with each
component separately, distances and speeds don't really give you what
you want. Rewrite this paragraph when the distributions are available.--Done
(by Tomomi)}

\textcolor{blue}{Note: There needs to be a short concluding discussion
of the halo matching across the 450/5 and 300/2 simulations stating
the actual halo numbers and why we are happy with using the 300/2
simulations up to this point.}

\textcolor{red}{As shown in Figure \ref{fig:mass-content}, the fraction
of unmatched halos in the 300/2 simulation to the 450/5 simulation
...
have the same mass as the ones in the 450/5 simulaion. Since the number
of sub-cylces do not affect to halo positions and velosities as shown
in Figure \ref{fig:HaloProperty_step}, the 300/2 timestep is our
choice to save the simulation time while keeping the halo properties
...
\caption{Values for amplitude $A$, expected value $\mu$, variance $\sigma$
in Gaussian fits shown in Figure \ref{fig:HaloProperty_step} for
the positions {[}right{]} and velocities {[}left{]} across simulations
...
\textcolor{blue}{NOTE: What other redshift values were used here to
determine these fits? What is the overall accuracy to which M\_re
now matches the halo masses for the 450/5 run?}

\textcolor{blue}{NOTE: STOP}
...
in Figure \ref{fig:nz_halo}. We compare original velocities computed
from the simulations in the right panel, while we multiply those velocity
by the expansion factor $a(z)$ in the left panel. By scaling with
the expansion factor, the velocity differences at different redshifts
become smaller. Figure XX shows the power spectra recomputed with
the velocity multiplied by the ratio of $a(z)/a(z=0.15)$, where $z$
is the redshift of the simulation. XXX}
...
\begin{figure}[H]
\includegraphics[width=1\columnwidth]{/Users/old_ts485/Dropbox/big-sims/Plots/test}
...
\textcolor{red}{I could not fix the x-label for the left panel:(}}
\end{figure}
...
\textcolor{red}{Finally, we compare halo number densities $n(z)$
as a function of redshift from the simulation at $z=0.55$ and the
lightcone output shown in Figure \ref{fig:nz_halo}. As expected,
$n(z)$ at $z=0.55$ is constant across redshift, while $n(z)$ from
the lightcone output decreases as redshift is larger. Figure \ref{fig:xi_halo}
shows correlation functions computed from the halos used in Figure
\ref{fig:xi_halo}. Even though $n(z)$ from two different outputs
are different, the correlation functions do not show any difference
in both real-space and redshift-space.}

\begin{figure}[H]
\includegraphics[width=0.5\columnwidth]{/Users/old_ts485/Dropbox/big-sims/Plots/nz_halo}

\caption{\label{fig:nz_halo}A comparison of halo number densities as a function
of redshift from the simulation at $z=0.55$ {[}blue{]} and the lightcone
output stitched every $\Delta z=0.05$ simulation {[}green{]}. We
use halos whose masses are greater than $10^{13.0}{\rm M_{\odot}}$.}
...
\begin{figure}[H]
\includegraphics[width=1\columnwidth]{/Users/old_ts485/Dropbox/big-sims/Plots/xi_halo}

\caption{\label{fig:xi_halo}Comparisons of halo correlation functions in real-space
{[}left{]} and redshift-space {[}right{]}. We use halos whose masses
are greater than $10^{13.0}{\rm M_{\odot}}$ from $z=0.4$ to $z=0.7$.
The blue lines are the correlation functions computed from the simulation
at $z=0.55$, while the green lines are from the lightcone output.
Even though $n(z)$ is different as shown in Figure \ref{fig:nz_halo},
the correlation functions show almost no difference.}
\end{figure}
...
how centrally concentrated the profile is. \textcolor{red}{$R_{vir}$
is computed from $M=\frac{4\pi}{3}R_{vir}^{3}\rho_{crit}\Delta_{h}$,
where $\Delta_{h}=(18\pi^{2}+82(\Omega_{m}-1)-39(\Omega_{m}-1)^{2})/\Omega_{m}$
and $\rho_{cris}=\frac{3c^{2}H_{0}^{2}}{8\pi G}$} ($c$ is the speed
of light here).There are several measurements of the dependence of
the concentration parameter $c$ on mass and redshift (\textcolor{red}{citations}).
We adopt the form in \cite{2011ApJ...740..102K}: 
\begin{equation}
...
shown in Figure \ref{fig:xis}. }\textcolor{red}{We compute $\chi^{2}$
for monopoles and quadrupoles separately for the correlation functions
from the simulation at $z=0.55$. We obtain $\chi^{2}=35.46$ for
monopole and $\chi^{2}=26.65$ for quadrupole. Note that we compute
$\chi^{2}$ with the covariance matrices where $s\in[48.,200.]$.
{*}need to do the same thing for the lightcone output...also vary
the range for s and recompute full covariancec matrix.}

\begin{figure}
\includegraphics[width=0.5\columnwidth]{/Users/old_ts485/Dropbox/big-sims/Plots/nz_dr11_1}
...
in text. \textcolor{red}{{*}need to add xi computed from lightcone
output.}}
\end{figure}
...
such as galaxy correlation functions. Through several convergence
tests described in Section 3, we find that systematic differences
on halo properties due to the approximated gravitational dynamics
give little effects on statistical observables. Especially, the agreement
between the full N-body simulation (i.e., the 450/5 simulation) and
the approximated simulations on halo bias is within 1\textasciitilde{}2\%
after the mass adjustment. In Section 4, we show how to construct
an output with smooth redshift distribution (i.e., lightcone). In
Section 5, we generate galaxy mocks from the sample of the 300/2 simulation
at $z=0.55$ and the lightcone output in order to compare our result
with the correlation function appeared in \cite{2013arXiv1303.4666A}.
The overall agreement on the scale between $30h^{-1}{\rm Mpc}$ and
$80h^{-1}{\rm Mpc}$ is well within the error in the measurement.
...
1/ Present the results of the time step increases : We present a quantitative
study of the impact of time step sizes on the halo and matter density
fields. Our code has two adjustable time stepping parameters - a global
time step and a number of sub-cycles (equivalent to a particle-particle
or tree force solve) to track that particle trajectories on small
scales (probably needs to be reworked slightly). We consider cases
...
fiducial choice is to use ... , resulting in a reduction of the simulation
run time by ???. We summarize the key results below : (a) The halo
masses tend to be underestimated in these cases, as one might expect
because ... . However, this trend may be calibrated with smaller simulations
and corrected, recovering the halo masses to <>\%. The halo mass function
is correctly recovered to <> for masses > <>. (a) The halo velocities
and positions are recovered to better than <> with a scatter of <>.
(a) The clustering of these halos is correctly recovered to better
than ??\% on scales below k < ???. (a) We find that the number of
sub-cycles makes almost no difference to any of our final results.

2/ Present the results of the light cone analysis : We also consider
the redshift sampling required to construct light cone outputs. In
real space, we find that moving halos over a \textbackslash{}Delta
z interval (at z=0.15) (is this the correct way to say this, or should
it be eg. \textbackslash{}delta t/age of Universe -- which I don't
think we hold constant) correctly recovers their positions to within
<>. The power spectra are correctly recovered to better than... for
k < ???. This worsens in redshift space to ...

We suggest shifting over redshift intervals no larger than..... Our
fiducial choice is ....

3/ Compare our approach to previous work

The biggest difference here is with LPT based approaches -- point
out differences. Point out that we are closer to the full N-body approach
but that we can gain significant time savings (and therefore run more
simulations) if our goal is to focus on large scales. These can be
done however controlling systematic effects.

4/ Present the BOSS simulations: As a representative example of our
method, we build a set of mock BOSS simulations. We have constructed
a set of <> mocks matching the CMASS galaxy sample.... <say a little
more here -- 2 complete North and South mocks per box, that we match
the large scale clustering.. > All the simulations (halo catalogs
etc) will be released with the publication of this paper. These simulations
are ideal for calibrating systematic errors, testing out full analysis
pipelines. These will be presented in future papers.

5/ Future simulations :

A short description here -- what we're running, where we got the time
from. Don't get into too many details, just point out highlights.
I'd crib from ALCC proposal here.
