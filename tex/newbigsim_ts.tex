...
system $\vec{x}_{new}$ such as $\vec{x}_{new}=R\vec{x}_{old}$, where
$R$ is the Euler rotation matrix: 
\[
...
$\gamma$ about the z-, x-, z-axis respectively.
...
without a central galaxy (\textcolor{red}{citations from LOWZ}). The
total number of galaxies hosted by each halo is a sum of the number
of central and satellite galaxies. Eqs \ref{eq:Ncen} and \ref{eq:Nsat}
...
BOSS galaxies (\textcolor{red}{citations}) and we use them in what
follows. 
