...
in this work to the Baryon Oscillation Spectroscopic Survey. Our N-body
simulations are consist of two components: a long time step for sloving
the PM force and a set of short range sub-cycle steps for a direct
particle-particle interactions. One of our goals is reducing the number
of time steps while keeping halo structure accurately. We quantitatively
evaluate the effects of reducing the number of time steps on halo
properties and found that we can reduce the simulation run time by
4 times less. We recover the halo masses to 98\% and the clustering
of halos less than 1\% on k < $1[h{\rm Mpc^{-1}]}$ in real-space.
We also show how to build light cone outputs with smooth redshift
distribution and how we can recover redshift-space by using velocity
field scaled by the expansion factor.}
...
almost Gaussian.} As is clear from Figure \ref{fig:HaloProperty_step},
...
simulations are started at \textcolor{red}{\$z=????\$} with Zel'dovich
initial conditions and are stopped at $z=0.15$ with simulation outputs
every $\Delta z=0.05$. At each of these steps, we store \textcolor{red}{??????}.
...
the concentration parameter $c$ on mass and redshift (\cite{2008JCAP...08..006M,2008MNRAS.390L..64D,2010arXiv1005.0411C,2011ApJ...740..102K,2013ApJ...766...32B,2001MNRAS.321..559B}).
...
\begin{figure}
\includegraphics[width=0.5\columnwidth]{/Users/old_ts485/Dropbox/big-sims/Plots/nz_dr11_N}\includegraphics[width=0.5\columnwidth]{/Users/old_ts485/Dropbox/big-sims/Plots/nz_dr11_S}
...
We present a quantitative study of the impact of time step sizes on
the halo and matter density fields. Our code has two adjustable time
stepping parameters - a global time step and a number of sub-cycles
(responsible for a particle-particle interactions) to track that particle
trajectories on small scales. We consider cases where we increase
the length of each time step by factors of 1.5 and 3 respectively,
as well as reducing the number of sub-cycles. Our fiducial choice
is to use using 300 global time steps and 2 sub-cycles (increasing
the length of the global time step by a factor of 1.5 and that of
the sub-cycles by a factor of 2.5), resulting in a reduction of the
simulation run time by 4 times less. We summarize the key results
below: 
...
recovered fully for masses above $10^{12.8}{\rm M_{\odot}}$ . 

(b) \textcolor{black}{The halo velocities and positions are recovered
...
We also consider the redshift sampling required to construct light
cone outputs. \textcolor{black}{In real space, we find that moving
halos over a $\Delta z=0.1$ interval correctly recovers their positions
from a scatter of $0.25[h^{-1}{\rm Mpc}]$ to $0.09[h^{-1}{\rm Mpc}]$.
The power spectra are correctly recovered to better than 1\% for k
< $0.5[h{\rm Mpc^{-1}]}$ for $\Delta z=0.25$. This worsens in redshift
space to 2\% up to k < $0.2[h{\rm Mpc^{-1}]}$. The agreement is,
however, improved to 1\% for k < $1[h{\rm Mpc^{-1}]}$ by using the
velocity scaled by the expansion factor $a(z)$. Our fiducial choice
to construct light cone outputs is $\Delta z=0.05$.}

\textcolor{black}{The biggest difference here with LPT based approaches
is the number of time steps and the short-range solvers using particle-particle
interactions. Our approach is closer to the full N-body simulation,
but we can gain significant time savings if our goal is to focus on
large scales. These can be done however controlling systematic effects.}

As a representative example of our method, we build a set of mock
BOSS simulations. We have constructed a set of 8 mocks matching the
CMASS galaxy sample: 2 complete North and South mocks per box for
four boxes. All the simulations will be released with the publication
of this paper. These simulations are ideal for calibrating systematic
errors, testing out full analysis pipelines. These will be presented
in future papers.

We will generate the simulation suite with the volume of $(4h^{-1}{\rm Gpc})^{3}$
with 68 billion particles each for 5 different cosmological models.
All the simulation will be run at Argonne National Laboratory. \textcolor{red}{(I
was not sure how much detail I should put here...)}
