...
capture the observational characteristics of surveys (e.Ourg. geometry,
...
on the halo density field. Section~\ref{sec:lightcones} discusses interpolating between
...
or $10^{12.6}$M$_{\odot}$. By comparison, the characteristic halo mass
of BOSS galaxies is $10^{13}$M$_{\odot}$, which we resolve with
...
\begin{equation}
\langle v_{x}^{2}\rangle=\langle v_{y}^{2}\rangle=\langle v_{z}^{2}\rangle=\frac{1}{3}\frac{GM}{R_{vir}}.\label{eq:variance}
\end{equation}
...
while the green and red line represents $n(z)$ from the mock catalog at $z=0.55$
...
computed from the full HOD galaxy mock catalog (labeled as ``data'') with
...
from $0.25h^{-1}{\rm Mpc}$ to}\textcolor{red}{{} }\textcolor{black}{$0.09h^{-1}{\rm Mpc}$.
